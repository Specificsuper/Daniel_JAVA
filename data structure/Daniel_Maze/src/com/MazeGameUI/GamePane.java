	...
	public void playGame() {
		...
		if(direct==Config.LEFT) {
			if(col-1>0&&Config.maze[row][col-1]!=Config.WALL) {
				gc.drawImage(LEFT[index], (col-1)*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
				fillBlock();
				index++;
				col--;
			}
			...
			if(col+1<n&&Config.maze[row][col+1]!=Config.WALL) {
				gc.drawImage(RIGHT[index], (col+1)*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
				fillBlock();
				index++;
				col++;
			}
			...
			if(row-1>0&&Config.maze[row-1][col]!=Config.WALL) {
				gc.drawImage(UP[index], col*Config.WIDTH, (row-1)*Config.WIDTH,Config.WIDTH,Config.WIDTH);
				fillBlock();
				index++;
				row--;
			}
			...
			if(row+1<n&&Config.maze[row+1][col]!=Config.WALL) {
				gc.drawImage(DOWN[index], col*Config.WIDTH, (row+1)*Config.WIDTH,Config.WIDTH,Config.WIDTH);
				fillBlock();
				index++;
				row++;
			}
	...
	public void initEntrance() {
		if(Config.maze[row][col]==Config.ENTRANCE) {
			if(col-1>0&&Config.maze[row][col-1]==Config.ROAD) {
				direct=Config.LEFT;
				gc.drawImage(LEFT[0], col*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
			}
			else if(col+1<n&&Config.maze[row][col+1]==Config.ROAD) {
				direct=Config.RIGHT;
				gc.drawImage(RIGHT[0], col*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
			}
			else if(row+1<n&&Config.maze[row+1][col]==Config.ROAD) {
				direct=Config.DOWN;
				gc.drawImage(DOWN[0], col*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
			}
			else if(row-1>0&&Config.maze[row-1][col]==Config.ROAD) {
				direct=Config.UP;
				gc.drawImage(UP[0], col*Config.WIDTH, row*Config.WIDTH,Config.WIDTH,Config.WIDTH);
			}
		}
